...
interface BatchDetailClientProps {
  id: string;
  initialBatch: Batch;
  initialMeatSales: MeatSale[];
  initialByproductSales: ByproductSale[];
  initialExpenses: Expense[];
  initialProfitData: ProfitCalculation;
  initialCustomerNames: string[];
}
...
export default function BatchDetailClient({
  ...
  initialBatch,
  initialMeatSales,
  initialByproductSales,
  initialExpenses,
  initialProfitData,
  initialCustomerNames,
}: BatchDetailClientProps) {
  const [batch, setBatch] = useState<Batch>(initialBatch);
  const [meatSales, setMeatSales] = useState<MeatSale[]>(initialMeatSales);
  const [byproducts, setByproducts] = useState<ByproductSale[]>(initialByproductSales);
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const [expenses, setExpenses] = useState<Expense[]>(initialExpenses);
  const [profit, setProfit] = useState<ProfitCalculation>(initialProfitData);
  ...
  const [customerNames, setCustomerNames] = useState<string[]>(initialCustomerNames);
  ...
  const [meatForm, setMeatForm] = useState({
    customerName: "",
    kgQuantity: "",
    pricePerKg: String(initialBatch.baseMeatPricePerKg),
    paidAmount: "",
  ...
  const [editForm, setEditForm] = useState({
    ...
    purchaseDate: new Date(initialBatch.purchaseDate).toISOString().split("T")[0],
    buyingCost: String(initialBatch.buyingCost),
    foodCost: String(initialBatch.foodCost),
    butcherCost: String(initialBatch.butcherCost),
    transportCost: String(initialBatch.transportCost),
    otherExpenses: String(initialBatch.otherExpenses),
    baseMeatPricePerKg: String(initialBatch.baseMeatPricePerKg),
    totalMeatKg: String(initialBatch.totalMeatKg),
  ...
  const handleAdjustStock = async (e: React.FormEvent) => {
    ...
    const json = await res.json();
    if (json.success) {
      toast.success("অবশিষ্ট গোশত ও মোট গোশত আপডেট হয়েছে!");
      setModal("");
      setLeftoverInput("");
      fetchData();
    } else {
      toast.error(json.error || "আপডেট করতে ব্যর্থ হয়েছে");
    }
  };
  ...
  // Dynamic import for PDF — only loaded when user clicks download
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      // Dynamic import — these heavy libraries are only loaded when needed
      ...
      const opt = {
        ...
        html2canvas: {
          ...
          onclone: (clonedDoc: Document) => {
            ...
            bodyStyle.setProperty("background-color", "#ffffff", "important");
  ...
  .print-header { page-break-inside: avoid !important; break-inside: avoid !important; }
  .no-print { display: none !important; width: 0 !important; height: 0 !important; padding: 0 !important; margin: 0 !important; overflow: hidden !important; border: none !important; }
  .data-table { width: 100%; border-collapse: collapse; table-layout: auto; }
  .data-table th { background-color: #f5f5f5 !important; padding: 12px 14px !important; text-align: left; font-weight: 700; border-bottom: 2px solid #cccccc; vertical-align: middle !important; line-height: 1.45 !important; display: table-cell !important; }
  .data-table td { padding: 12px 14px !important; text-align: left; border-bottom: 1px solid #e5e5e5; vertical-align: middle !important; line-height: 1.45 !important; display: table-cell !important; }
  .data-table tfoot td { background-color: #f5f5f5 !important; font-weight: 700; border-top: 2px solid #cccccc; vertical-align: middle !important; line-height: 1.45 !important; padding: 12px 14px !important; display: table-cell !important; }
  .data-table tfoot td.tfoot-total { background-color: #e0f2fe !important; }
  .data-table tfoot td.tfoot-paid { background-color: #dcfce7 !important; }
  .data-table tfoot td.tfoot-due { background-color: #fee2e2 !important; }
  .data-table td > div { display: block !important; margin: 0 !important; padding: 0 !important; }
  .badge { background: transparent !important; border: none !important; box-shadow: none !important; color: #000000 !important; padding: 0 !important; margin: 0 8px 0 0 !important; font-size: 14px !important; font-weight: 500 !important; display: inline-block !important; line-height: 1.45 !important; vertical-align: middle !important; }
  .badge:not(:last-child)::after { content: ", " !important; }
  .glass-card { border: 1px solid #e0e0e0 !important; border-radius: 8px !important; overflow: hidden !important; margin-bottom: 1rem !important; box-shadow: none !important; }
`;
            clonedDoc.head.appendChild(styleEl);

            [".sticky-batch-header", ".desktop-batch-header", ".tab-nav", ".fab-container", ".no-print", ".mobile-sale-cards"].forEach((sel: string) => {
              clonedDoc.querySelectorAll(sel).forEach((el: Element) => el.remove());
            });
            ...
            clonedDoc.querySelectorAll("*").forEach((el: Element) => {
              ...
              if (!el.classList.contains("tfoot-paid") && !el.classList.contains("tfoot-due") && !el.classList.contains("td-paid") && !el.classList.contains("td-due")) {
      ...
      toast.error("সরাসরি ডাউনলোড ব্যর্থ হয়েছে। বিকল্প পদ্ধতিতে প্রিন্ট ডায়ালগ চালু হচ্ছে...", { duration: 4000 });
      setTimeout(() => {
        window.print();
      }, 1000);
    } finally {
  ...
  // Use CSS media queries for icon size instead of JS resize listener
  const iconSize = 20;
    ...
    <PullToRefresh onRefresh={fetchData}>
      <div className="animate-fade-in">
        ...
        <div className="print-header">
          ...
          <p style={{ fontSize: "1rem", fontWeight: 600, marginBottom: "0.15rem" }}>
            {batch.batchName}
          </p>
          <p style={{ fontSize: "0.85rem" }}>
            জবাইয়ের তারিখ:{" "}
            {formatBengaliDate(batch.purchaseDate, { day: "numeric", month: "long", includeYear: true })}
          </p>
        </div>
        ...
        <div className="sticky-batch-header">
          <div className="batch-title" style={{ justifyContent: "space-between", width: "100%" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
              <Link href="/batches" style={{ color: "var(--text-muted)", display: "flex" }}>
            ...
            <div style={{ display: "flex", gap: "0.5rem" }}>
              {tab === "sales" && (
                ...
                >
                  <HiOutlinePrinter size={mobileIconSize} />
                </button>
              )}
              <button
                className="btn-icon"
                ...
                style={{ padding: "0.25rem", color: "var(--text-primary)" }}
              >
                <HiOutlinePencil size={mobileIconSize} />
              </button>
            </div>
          </div>
        ...
        <Link
          ...
          style={{ display: "inline-flex", alignItems: "center", gap: "0.5rem", color: "var(--text-muted)", textDecoration: "none", fontSize: "0.85rem", marginBottom: "0.75rem" }}
        >
        ...
        >
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
            {tab === "sales" && (
              <button className="btn btn-secondary btn-sm" onClick={() => setModal("pdfOptions")}>
        ...
        >
            ...
            <div className="animate-fade-in">
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "1rem", marginBottom: "1.5rem" }}>
                <div className={`stat-card ${profit.netProfit >= 0 ? "green" : "red"}`}>
                  ...
                  <div style={{ fontSize: "0.72rem", opacity: 0.7, marginTop: "0.2rem" }}>{toBengaliDigits(profit.profitMargin.toFixed(1))}% মার্জিন</div>
                </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: "1.25rem" }}>
                ...
                <div className="glass-card" style={{ padding: "1.5rem" }}>
                  ...
                  {[
                    ...
                    { l: "চামড়া/ভুঁড়ি/পা বিক্রি", v: profit.totalByproductRevenue },
                  ].map((r) => (
                ...
                <div className="glass-card" style={{ padding: "1.5rem" }}>
                  ...
                  {([
                    { l: "মোট গোশত", v: `${toBengaliDigits(batch.totalMeatKg)} কেজি` },
                    { l: "বিক্রি হয়েছে", v: `${toBengaliDigits(profit.totalKgSold.toFixed(1))} কেজি` },
                    {
                      l: "বাকি আছে",
                      v: (
                        <span style={{ display: "inline-flex", alignItems: "center", gap: "0.5rem" }}>
                          <span>{toBengaliDigits(profit.remainingKg.toFixed(1))} কেজি</span>
                          <button onClick={() => { setLeftoverInput(profit.remainingKg > 0 ? profit.remainingKg.toFixed(1) : "0"); setModal("adjustStock"); }}
                            style={{ background: "none", border: "none", color: "var(--accent-blue)", cursor: "pointer", padding: "2px", display: "inline-flex", alignItems: "center", justifyContent: "center", borderRadius: "4px", transition: "all 0.2s" }}
                    ...
                    { l: "স্টক মূল্য", v: fmt(Math.round(profit.stockValue)), color: "var(--accent-blue)" },
                    { l: "লাভ/কেজি", v: (profit.profitPerKg >= 0 ? "+" : "") + fmt(Math.round(profit.profitPerKg)), color: profit.profitPerKg >= 0 ? "var(--accent-green)" : "var(--accent-red)" },
                    { l: "বেস দাম/কেজি", v: fmt(batch.baseMeatPricePerKg) },
                    { l: "আনুমানিক মোট লাভ", v: (profit.projectedProfit >= 0 ? "+" : "") + fmt(Math.round(profit.projectedProfit)), color: profit.projectedProfit >= 0 ? "var(--accent-green)" : "var(--accent-red)" },
                  ] as { l: string; v: string | React.ReactNode; color?: string }[]).map((r) => (
                    <div key={r.l} style={{ display: "flex", justifyContent: "space-between", padding: "0.65rem 0", borderBottom: "1px solid var(--border-color)" }}>
                      <span style={{ color: "var(--text-secondary)", fontSize: "0.875rem" }}>{r.l}</span>
                      <span style={{ fontWeight: 600, color: r.color || "var(--text-primary)" }}>{r.v}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {allDues.length > 0 && (
                <div className="glass-card" style={{ padding: "1.5rem", marginTop: "1.25rem" }}>
                  <h3 style={{ fontSize: "1rem", fontWeight: 700, marginBottom: "1rem", color: "var(--accent-yellow)" }}>
          ...
          {tab === "sales" &&
            (() => {
              ...
              const grandKgSold = filteredMeat.reduce((sum, s) => sum + s.kgQuantity, 0);
                    ...
                    <div style={{ display: "flex", gap: "0.5rem", alignItems: "center" }}>
                      {(meatSales.length > 0 || byproducts.length > 0) && (
                        <div style={{ position: "relative" }}>
                          <HiOutlineSearch size={16} style={{ position: "absolute", left: 10, top: 9, color: "var(--text-muted)" }} />
                          <input className="form-input" value={searchSales} onChange={(e) => setSearchSales(e.target.value)} placeholder="ক্রেতা খুঁজুন..." style={{ paddingLeft: "2rem", width: 180, height: 36, fontSize: "0.8rem" }} />
                        </div>
                      )}
                      <button className="btn btn-primary btn-sm" onClick={() => { setSaleType("meat"); setModal("sale"); }}>
                        + বিক্রি যোগ করুন
                      </button>
                    </div>
                  </div>
                    ...
                    {customerRows.length === 0 ? (
                      ...
                      <>
                        ...
                        <table className="data-table desktop-table">
                          ...
                          <tfoot>
                            <tr>
                              <td>মোট ({toBengaliDigits(customerRows.length)} জন)</td>
                              <td>{toBengaliDigits(grandKgSold.toFixed(1))} কেজি</td>
                              <td style={{ fontWeight: 700 }} className="tfoot-total">{fmt(grandTotal)}</td>
                              <td style={{ color: "var(--accent-green)", fontWeight: 700 }} className="tfoot-paid">{fmt(grandPaid)}</td>
                              <td style={{ color: "var(--accent-red)", fontWeight: 700 }} className="tfoot-due">{fmt(grandDue)}</td>
                        ...
                        <div className="mobile-sale-cards">
                            ...
                            <div key={c.name} className="sale-card">
                              ...
                              <div className="sale-card-grid">
                                <div className="sale-card-stat">
                                  ...
                                  <div className="sale-card-stat-value">{fmt(c.totalAmount)}</div>
                                </div>
                                <div className="sale-card-stat">
                                  <div className="sale-card-stat-label">পেইড</div>
                                  <div className="sale-card-stat-value" style={{ color: "var(--accent-green)" }}>{fmt(c.totalPaid)}</div>
                                </div>
                                <div className="sale-card-stat">
                                  <div className="sale-card-stat-label">বাকি</div>
                                  <div className="sale-card-stat-value" style={{ color: c.totalDue > 0 ? "var(--accent-red)" : "var(--accent-green)" }}>{fmt(c.totalDue)}</div>
                                </div>
                              </div>
                          ...
                          <div style={{ padding: "0.5rem", background: "rgba(16,185,129,0.06)", borderRadius: "10px", display: "flex", justifyContent: "space-between", fontSize: "0.8rem", fontWeight: 700 }}>
                            <span>মোট: {toBengaliDigits(customerRows.length)} জন কাস্টমার</span>
          ...
          {saleType === "meat" ? (
            <form onSubmit={addMeatSale}>
              <div className="form-group">
                ...
                {showSuggestions === "meat" && getSuggestions(meatForm.customerName).length > 0 && typeof window !== "undefined" &&
                  createPortal(
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem" }}>
                ...
                <div className="form-group">
                  ...
                  <input type="date" className="form-input" value={meatForm.date} onChange={(e) => setMeatForm({ ...meatForm, date: e.target.value })} />
                </div>
              </div>
              {meatForm.kgQuantity && meatForm.pricePerKg && (
                <div style={{ padding: "0.75rem", background: "var(--bg-secondary)", borderRadius: "10px", marginBottom: "1rem", fontSize: "0.85rem" }}>
                  মোট: <strong>{fmt(Number(meatForm.kgQuantity) * Number(meatForm.pricePerKg))}</strong> | বাকি: <strong style={{ color: "var(--accent-yellow)" }}>{fmt(Number(meatForm.kgQuantity) * Number(meatForm.pricePerKg) - (Number(meatForm.paidAmount) || 0))}</strong>
                </div>
              )}
            ...
            <form onSubmit={(e) => { setBypForm((f) => ({ ...f, itemType: saleType })); addByproduct(e); }}>
              ...
              <div className="form-group">
                ...
                {showSuggestions === "byp" && getSuggestions(bypForm.buyerName).length > 0 && typeof window !== "undefined" &&
                  createPortal(
                    <div style={suggestionStyle}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem" }}>
                ...
                <div className="form-group">
                  <label className="form-label">তারিখ</label>
                  <input type="date" className="form-input" value={bypForm.date} onChange={(e) => setBypForm({ ...bypForm, date: e.target.value })} />
                </div>
              </div>
              {bypForm.price && (
                <div style={{ padding: "0.75rem", background: "var(--bg-secondary)", borderRadius: "10px", marginBottom: "1rem", fontSize: "0.85rem" }}>
                  মোট: <strong>{fmt(Number(bypForm.quantity || 1) * Number(bypForm.price))}</strong> | বাকি: <strong style={{ color: "var(--accent-yellow)" }}>{fmt(Number(bypForm.quantity || 1) * Number(bypForm.price) - (Number(bypForm.paidAmount) || 0))}</strong>
                </div>
              )}
          ...
          <form onSubmit={addExpense}>
            <div className="form-group">
              ...
              <select className="form-select" value={expForm.expenseType} onChange={(e) => setExpForm({ ...expForm, expenseType: e.target.value })}>
                <option value="food">খাবার</option><option value="butcher">কসাই</option><option value="transport">পরিবহন</option><option value="medicine">ওষুধ</option><option value="other">অন্যান্য</option>
              </select>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem" }}>
        ...
        <Modal isOpen={modal === "adjustStock"} onClose={() => setModal("")} title="অবশিষ্ট গোশত (স্টক) সংশোধন">
          {profit && (
            <form onSubmit={handleAdjustStock}>
              <div style={{ padding: "1rem", background: "var(--bg-secondary)", borderRadius: "10px", marginBottom: "1.25rem", fontSize: "0.9rem", lineHeight: "1.6", border: "1px solid var(--border-color)" }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "0.5rem" }}><span style={{ color: "var(--text-secondary)" }}>ইতিমধ্যে বিক্রি হয়েছে:</span><span style={{ fontWeight: 600, color: "var(--text-primary)" }}>{toBengaliDigits(profit.totalKgSold.toFixed(1))} কেজি</span></div>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "0.5rem" }}><span style={{ color: "var(--text-secondary)" }}>বর্তমান মোট গোশত:</span><span style={{ fontWeight: 600, color: "var(--text-primary)" }}>{toBengaliDigits((batch?.totalMeatKg || 0).toFixed(1))} কেজি</span></div>
                <div style={{ display: "flex", justifyContent: "space-between", borderTop: "1px solid var(--border-color)", paddingTop: "0.5rem", marginTop: "0.5rem" }}><span style={{ color: "var(--text-secondary)" }}>হিসাবকৃত অবশিষ্ট:</span><span style={{ fontWeight: 600, color: "var(--text-primary)" }}>{toBengaliDigits(profit.remainingKg.toFixed(1))} কেজি</span></div>
              </div>
              ...
              <div style={{ display: "flex", gap: "0.75rem" }}>
                <button type="button" className="btn" onClick={() => setModal("")} style={{ flex: 1, background: "var(--bg-secondary)", border: "1px solid var(--border-color)", color: "var(--text-primary)" }}>বাতিল করুন</button>
                <button type="submit" className="btn btn-primary" style={{ flex: 1 }}>আপডেট করুন</button>
              </div>
            </form>
          )}
        </Modal>
        ...
        <Modal isOpen={modal === "pdfOptions"} onClose={() => setModal("")} title="রিপোর্ট ডাউনলোড / প্রিন্ট">
          <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
            <p style={{ fontSize: "0.9rem", color: "var(--text-secondary)", lineHeight: "1.5" }}>আপনার ডিভাইসের জন্য সুবিধাজনক পদ্ধতিটি নির্বাচন করুন:</p>
            <button type="button" onClick={async () => { setModal(""); await new Promise((resolve) => setTimeout(resolve, 300)); handleDownloadPdf(); }} style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", padding: "1rem 1.25rem", background: "linear-gradient(135deg, rgba(16, 185, 129, 0.1) 0%, rgba(5, 150, 105, 0.05) 100%)", border: "1px solid rgba(16, 185, 129, 0.2)", borderRadius: "12px", cursor: "pointer", textAlign: "left", width: "100%", transition: "all 0.2s ease" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", fontWeight: 700, color: "var(--accent-green)", fontSize: "1rem", marginBottom: "0.35rem" }}>📥 সরাসরি PDF ডাউনলোড (Direct Download)</div>
              <div style={{ fontSize: "0.8rem", color: "var(--text-secondary)", lineHeight: "1.4" }}>মোবাইলে সরাসরি PDF ফাইল নামিয়ে নিন। বেশিরভাগ ব্রাউজারে এটি ভালো কাজ করে।</div>
            </button>
            <button type="button" onClick={() => { setModal(""); setTimeout(() => { window.print(); }, 350); }} style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", padding: "1rem 1.25rem", background: "linear-gradient(135deg, rgba(59, 130, 246, 0.1) 0%, rgba(37, 99, 235, 0.05) 100%)", border: "1px solid rgba(59, 130, 246, 0.2)", borderRadius: "12px", cursor: "pointer", textAlign: "left", width: "100%", transition: "all 0.2s ease" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", fontWeight: 700, color: "var(--accent-blue)", fontSize: "1rem", marginBottom: "0.35rem" }}>🖨️ সিস্টেম প্রিন্ট / PDF সংরক্ষণ (Print / Save as PDF)</div>
              <div style={{ fontSize: "0.8rem", color: "var(--text-secondary)", lineHeight: "1.4" }}>যাদের ফোনে সরাসরি ডাউনলোড কাজ করে না (যেমন: Xiaomi, Huawei, Oppo, Vivo, বা যেকোনো mobile WebView), তারা এই অপশনটি দিয়ে সহজেই <strong>Save as PDF</strong> বা প্রিন্ট করতে পারবেন।</div>
            </button>
            <button type="button" className="btn" onClick={() => setModal("")} style={{ marginTop: "0.5rem", background: "var(--bg-secondary)", border: "1px solid var(--border-color)", color: "var(--text-primary)", width: "100%" }}>বাতিল করুন</button>
          </div>
        </Modal>
